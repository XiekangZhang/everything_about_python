{% macro create_model_yml(model_name) %}
    ...
    {% set model_name = meta.model_name %}
    {% if not model_name %}
        {{ exceptions.raise_compiler_error("ERROR: Model Name is mandatory, but got: " ~ model_name) }}
    {% endif %}
...
# To make changes, edit the `meta` block in the model sql file and ru-run this macro.         #
...
{{ level1 }}description: {% if '"' in model_description -%} {{ "'" ~ model_description ~ "'" }} {%- else -%} {{ model_description }} {%- endif %}
{{ level1 }}config:
{{ level2 }}tags: {{ tags | string | replace("'", '"') | replace("'", '"') }} 
...
{{ level1 }}columns:
{%- for col in cols_tests_config.keys() %}
{{ level2 }}- name: {{ col }}
{%- if cols.get(col).description %}
{{ level3 }}description: {% if '"' in cols.get(col).description -%} {{ "'" ~ cols.get(col).description ~ "'" }} {%- else -%} {{ cols.get(col).description }} {%- endif %}
{%- endif -%}
...
:
{%- if "value" in test_config.keys() or "inclusive" in test_config.keys() or "values" in test_config.keys() -%}
